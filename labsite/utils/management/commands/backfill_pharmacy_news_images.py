...
class Command(BaseCommand):
    help = "Backfill listing images for pharmacy news from source article pages."
    ...
    def handle(self, *args, **options):
        queryset = ArticlePage.objects.filter(topic__slug="pharmacy-news").specific()
        if not options["overwrite"]:
            queryset = queryset.filter(listing_image__isnull=True)
        queryset = queryset.order_by("-publication_date", "-first_published_at")
        if options["limit"]:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Backfilled pharmacy news images: "
    ...
    def import_image(self, url):
        ...
        existing = CustomImage.objects.filter(title=f"Pharmacy news {digest}").first()
        if existing:
            return existing
        ...
        image = CustomImage(title=f"Pharmacy news {digest}")
        image.file.save(f"pharmacy-news-{digest}{suffix}", ContentFile(content), save=False)
        image.width = width
        image.height = height
        image.save()
        return image
